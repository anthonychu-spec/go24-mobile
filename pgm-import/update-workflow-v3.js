...
console.log('Updating Extract Message code only (no Gemini key changes)...');
...
// Fix Send WhatsApp Reply to use Google Sheets2 (reference the working node)
...
// Modify Reply: Help Menu to send interactive buttons
...
console.log('Reply: Help Menu updated to send buttons');
  ...
  res.on('end', () => {
    ...
    if (res.statusCode >= 200 && res.statusCode < 300) {
      try {
        ...
        console.log('\nChanges made:');
        console.log('1. Extract Message: now handles button_reply messages');
        console.log('2. Send WhatsApp Reply: fixed token reference');
        console.log('3. Reply: Help Menu: now sends interactive buttons instead of text');
        console.log('\n⚠️  Still need to:');
        console.log('- Add session table check node');
        console.log('- Add upsert session node');
      } catch (e) {
